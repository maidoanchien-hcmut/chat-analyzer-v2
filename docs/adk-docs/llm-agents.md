...
_(Note: For instructions that apply to_ all _agents in a system, consider using `global_instruction` on the root agent, detailed further in the [Multi-Agents](https://google.github.io/adk-docs/agents/multi-agents/index.md) section.)_
...
const capitalAgent = new LlmAgent({
  ...
  description:
    "Answers user questions about the capital city of a given country.",
  instruction:
    "You are an agent that provides the capital city of a country...", // Note: the full instruction is omitted for brevity
