...
| `organization_id` | `text` | Không | index | Chủ sở hữu page; có thể default `default_platform` trong giai đoạn đầu |
| `pancake_page_id` | `text` | Không | unique | ID page thật từ Pancake |
| `page_name` | `text` | Không | index | Tên page lấy từ Pancake lúc register |
| `pancake_user_access_token_ciphertext` | `text` | Không |  | User access token dùng để generate page access token |
...
| `cloned_from_page_id` | `uuid` | Có | FK | Prompt này clone từ page nào |
| `cloned_from_prompt_version_id` | `uuid` | Có | FK | Prompt này clone từ version nào |
