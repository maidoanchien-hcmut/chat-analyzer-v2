...
- Cho phép chỉnh sửa model của mỗi agent trong luồng adk.
...
8. Dev chỉnh các thông số và prompt:
   ...
   8.2. Prompt phải có sẵn placeholder dùng như thật. Có tính năng clone lại prompt cũ, prompt của các trang khác để chỉnh sửa nhanh. Có tính năng test prompt với dữ liệu runtime đã lấy ở bước 7, xem kết quả ngay trên giao diện để chỉnh sửa thêm.
