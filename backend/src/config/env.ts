...
const envSchema = z
  .object({
    ...
    ANALYSIS_SERVICE_BASE_URL: z.string().url().default("http://127.0.0.1:8000"),
    ANALYSIS_SERVICE_TIMEOUT_MS: z.coerce.number().int().positive().default(60000),
    ANALYSIS_SERVICE_SHARED_SECRET: z.string().min(1).default("dev-only-insecure-change-me"),
  ...
  .transform((raw) => {
    ...
    return {
      ...
      host: raw.HOST,
      port: raw.PORT,
      analysisServiceBaseUrl: raw.ANALYSIS_SERVICE_BASE_URL,
      analysisServiceTimeoutMs: raw.ANALYSIS_SERVICE_TIMEOUT_MS,
      analysisServiceSharedSecret: raw.ANALYSIS_SERVICE_SHARED_SECRET,
      databaseUrl,
      redisUrl,
